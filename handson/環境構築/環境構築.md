...
>
> AI Search のインデックス作成でエラーが発生した場合は、再実行前に Azure ポータルから以下のリソースを手動で削除してください。
>
> 1. Azure ポータルで AI Search サービスを開く
> 2. 以下の順番で各リソースを削除する（依存関係のため逆順に削除）
>    - **インデクサー**: `indexer-noacl-gen2`、`indexer-acl-gen2`
>    - **スキルセット**: `skill-noacl-gen2`、`skill-acl-gen2`
>    - **インデックス**: `index-noacl-gen2`、`index-acl-gen2`
>    - **データソース**: `ds-noacl-gen2`、`ds-acl-gen2`
>
> 削除後、再度 `azd up` を実行してください。
