...
`telemetry_utils.py` の `inject_traceparent()` が、OpenTelemetry の標準 propagator を使用して W3C TraceContext をリクエストヘッダーに注入します。
...
この関数を httpx の `event_hooks` に登録することで、すべての HTTP リクエストに自動で `traceparent` ヘッダー（および必要に応じて `tracestate`）が付与されます。

**OpenTelemetry propagator を使用する利点:**

- sampled flag（トレースフラグ）が親スパンの状態に基づいて正しく設定される
- `tracestate` ヘッダーも自動的に伝播される
- W3C TraceContext 仕様に完全準拠した実装が保証される
