...
`appcodes/other/` ディレクトリには、以下のユーティリティスクリプトが用意されています。

- **`hello.py`**: シンプルな挨拶スクリプト
  ```bash
  python3 appcodes/other/hello.py
  ```
  
- **`check.entraid_token.py`**: Entra ID トークンの内容を確認するスクリプト

- **`test_language-service-pii.py`**: Azure Language Service の PII 検出機能をテストするスクリプト
